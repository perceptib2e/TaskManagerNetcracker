...
public class Tasks {
    ...
    public static Iterable<Task> incoming(Iterable<Task> tasks, Date start, Date end) throws CustomExeption {
    ...
    public static SortedMap<Date, Set<Task>> calendar (Iterable<Task> tasks, Date start, Date end) throws CustomExeption {
        ...
        //идём по списку и добавляем все задачи, время которых
        for (Task task : tempListOfTasks){
            //создаём время от которого ищем задачу
            Date date = task.nextTimeAfter(start);
            //если время соответствует условию (не больше границ calendar)
